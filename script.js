...
function generateFutureGrid() {
    ...
    const positions = [
        ...
        },
        {
            type: 'SELL STOP',
            entryPrice: marketPrice,
            lowerPrice: marketPrice * (1 + interval),   // ✅ Inversé
            upperPrice: marketPrice * (1 - interval),   // ✅ Inversé
            stake: stakes[1],
...
function renderFutureTable(positions, container) {
    ...
    table.classList.add('results-table');
    const headerRow = document.createElement('tr');
    headerRow.innerHTML = `<th colspan="6" class="block-header">Bloc de Trading Future - 3 Positions</th>`;
    table.appendChild(headerRow);
...
// === Fonctions d’archivage, prêt, profit, etc. ===
// (identiques à l’original, inchangées)
