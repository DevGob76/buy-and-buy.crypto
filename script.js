...
const translations = {
    fr: {
        ...
        gridCalculator: "Calculateur de Grille Crypto",
        ...
        archiveDataTitle: "Données de l'archive sélectionnée",
    ...
    en: {
        ...
        gridCalculator: "Crypto Grid Calculator",
        ...
        archiveDataTitle: "Selected Archive Data",
    ...
    es: {
        ...
        gridCalculator: "Calculadora de Cuadrícula de Criptomonedas",
        ...
        archiveDataTitle: "Datos del Archivo Seleccionado",
        calendarTitle: "📅 Calendario Crypto",
        calendarSubtitle: "Calendario Alcista y Bajista (12 Meses)",
        bullishCalendarTitle: "Calendario Alcista de 3 x 6 Meses",
    ...
    de: {
        ...
        gridCalculator: "Krypto-Gitter-Rechner",
        ...
        archiveDataTitle: "Daten des ausgewählten Archivs",
...
async function downloadArchiveAsPDF() {
    ...
    if (!archiveContent) {
        showMessagePopup('Erreur', 'Aucune archive sélectionnée.');
        return;
    }
...
function generateCalendar() {
    ...
    if (!startDateInput) {
        // Don't show an alert on initial load.
        return;
    }
    ...
    for (let month = 0; month < 12; month++) {
        ...
        bearishStart.setDate(bearishStart.getDate() + 1); // Start next day
        const bearishEnd = new Date(bearishStart);
        bearishEnd.setDate(bearishStart.getDate() + 6);
        
        const monthDiv = document.createElement('div');
        monthDiv.className = 'calendar-month';

        const monthTitle = document.createElement('h4');
        monthTitle.textContent = monthStart.toLocaleString('default', { month: 'long', year: 'numeric' });
        monthDiv.appendChild(monthTitle);
        
        const bullishDiv = document.createElement('div');
        bullishDiv.className = 'calendar-cell bullish';
        bullishDiv.innerHTML = `
            <div>Début Haussier: ${bullishStart.toLocaleDateString()}</div>
            <div>Fin Haussier: ${bullishEnd.toLocaleDateString()}</div>
        `;
        ...
        bearishDiv.innerHTML = `
            <div>Début Baissier: ${bearishStart.toLocaleDateString()}</div>
            <div>Fin Baissier: ${bearishEnd.toLocaleDateString()}</div>
        `;
...
function updateDebtRatio() {
    ...
    if (collateralBudget + loanAmount === 0) {
        document.getElementById('debt-ratio').value = (0).toFixed(2);
        return;
    }
...
function loadProfitData() {
    if (profitData.rows && profitData.rows.length > 0) {
        const tableBody = document.getElementById('profit-table-body');
        tableBody.innerHTML = '';
        profitData.rows.forEach((rowData, index) => {
            ...
            newRow.innerHTML = `
                <td>${index + 1}</td>
                <td><input type="date" value="${rowData.startDate || ''}" onchange="saveProfitData()"></td>
                <td><input type="date" value="${rowData.endDate || ''}" onchange="saveProfitData()"></td>
                <td><input type="number" value="${rowData.startBudget || 300}" onchange="saveProfitData()"></td>
                <td><input type="number" value="${rowData.endBudget || 450}" onchange="saveProfitData()"></td>
                <td><input type="number" readonly></td>
                <td><input type="number" readonly></td>
            `;
            tableBody.appendChild(newRow);
...
function changeLanguage() {
    ...
    // Header
    setText('.header h1', 'welcome');
    setText('.header p', 'appDescription');

    // Nav
    const navButtons = document.querySelectorAll('.nav-btn');
    if (navButtons.length === 5) {
        navButtons[0].textContent = t.home;
        navButtons[1].textContent = t.distribution;
        navButtons[2].textContent = t.calendar;
        navButtons[3].textContent = t.loan;
        navButtons[4].textContent = t.profit;
    }
    
    // Page Accueil
    setText('#accueil h2', 'welcome');
    setText('#accueil .auth-form h3', 'userAccess');
    setText('#accueil .form-group label', 'phoneNumber');
    setText('#accueil .btn', 'access');
    
    // Page Répartition
    setText('#repartition h2', 'gridCalculator');
    setText('label[for="crypto-select"]', 'cryptoCurrency');
    setText('label[for="initial-price"]', 'initialPrice');
    setText('label[for="budget"]', 'budget');
    setText('label[for="rate-1"]', 'stakeRate1');
    setText('label[for="rate-2"]', 'stakeRate2');
    setText('label[for="rate-3"]', 'stakeRate3');
    setText('label[for="rate-4"]', 'stakeRate4');
    setText('label[for="grid-interval"]', 'gridInterval');
    setText('label[for="trigger-drop"]', 'triggerDrop');
    setText('label[for="num-blocks"]', 'numBlocks');
    setText('#generate-grid-btn', 'generateGrid');
    setText('label[for="archive-date"]', 'archiveDate');
    document.querySelectorAll('#repartition .archive-actions .btn')[0].textContent = t.archiveDistribution;
    setText('#download-pdf-btn', 'downloadPDF');
    document.querySelectorAll('#repartition .archive-actions .btn')[1].textContent = t.deleteArchive;
    setText('label[for="archive-select"]', 'selectArchive');
    setText('#archive-display h3', 'archiveDataTitle');
    
    // Page Calendrier
    setText('#calendrier h2', 'calendarTitle');
    setText('#calendrier .calendar-section:nth-child(1) .calendar-title', 'calendarSubtitle');
    setText('label[for="calendar-start-date"]', 'calendarStartDate');
    setText('#calendrier .calendar-section:nth-child(2) .calendar-title', 'bullishCalendarTitle');
    setText('#calendrier .calendar-section:nth-child(2) p', 'bullishCalendarDesc');
    setText('label[for="days-input"]', 'daysInput');
    setText('label[for="crypto-price"]', 'cryptoPrice');
    const thsBullish = document.querySelectorAll('#bullish-calendar th');
    if (thsBullish.length === 4) {
        thsBullish[0].textContent = t.ref;
        thsBullish[1].textContent = t.probableDate;
        thsBullish[2].textContent = t.degressiveRate;
        thsBullish[3].textContent = t.bullishPoint;
    }

    // Page Prêt
    setText('#pret-trading h2', 'loanTitle');
    const loanLabels = document.querySelectorAll('#pret-trading .trading-grid .trading-cell label');
    if (loanLabels.length >= 8) {
        loanLabels[0].textContent = t.collateralBudget;
        loanLabels[1].textContent = t.recommendedLoanRatio;
        loanLabels[2].textContent = t.recommendedLoanAmount;
        loanLabels[3].textContent = t.debtRatio;
        loanLabels[4].textContent = t.dailyInterestRate;
        loanLabels[5].textContent = t.loanDuration;
        loanLabels[6].textContent = t.totalInterest;
        loanLabels[7].textContent = t.totalRepayment;
    }
    setText('label[for="loan-archive-date"]', 'archiveDate');
    document.querySelector('#pret-trading .archive-actions .btn').textContent = t.archiveLoan;
    setText('label[for="loan-archive-select"]', 'selectArchive');
    document.querySelector('#pret-trading .archive-actions .btn:last-of-type').textContent = t.deleteLoanArchive;
    
    // Page Bilan
    setText('#bilan-profit h2', 'profitTitle');
    setText('label[for="total-profit"]', 'totalProfit');
    setText('label[for="profit-rate"]', 'profitRate');
    setText('label[for="initial-capital"]', 'initialCapital');
    document.querySelector('#bilan-profit .trading-cell .btn').textContent = t.deleteData;
    const thsProfit = document.querySelectorAll('.profit-table th');
    if (thsProfit.length === 7) {
        thsProfit[0].textContent = t.profitTable.no;
        thsProfit[1].textContent = t.profitTable.startDate;
        thsProfit[2].textContent = t.profitTable.endDate;
        thsProfit[3].textContent = t.profitTable.startBudget;
        thsProfit[4].textContent = t.profitTable.endBudget;
        thsProfit[5].textContent = t.profitTable.profit;
        thsProfit[6].textContent = t.profitTable.profitRate;
    }
    document.querySelector('#bilan-profit > .btn').textContent = t.addLine;
    
    // Admin
    setText('#secret-code-popup h3', 'adminCode');
    setText('#secret-code-popup label', 'secretCode');
    document.querySelector('#admin-secret').placeholder = t.enterSecretCode;
    setText('#secret-code-popup .btn', 'adminAccess');
    setText('#parametres h2', 'adminSettings');
    setText('#parametres h3', 'addUser');
    setText('label[for="admin-user-name"]', 'fullName');
    setText('label[for="admin-user-phone"]', 'phone');
    document.querySelector('#parametres .btn').textContent = t.addUserBtn;
    
    // Popup
    setText('#popup-title', 'info');
    setText('#message-popup .btn', 'close');
}
...
function checkAccess(pageId, element) {
    if (isAuthenticated) {
...
function checkAdminAccess() {
    ...
    if (secretCode === adminSecretCode) {
        ...
        showPage('parametres', null); // Don't highlight a nav button
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loanInputs = document.querySelectorAll('#pret-trading input[type=number]:not([readonly])');
    loanInputs.forEach(input => {
        input.addEventListener('input', updateAllCalculations);
    });
    updateAllCalculations(); 
    document.getElementById('initial-capital').addEventListener('input', saveProfitData);
    document.getElementById('crypto-price').addEventListener('input', updateDates);
});