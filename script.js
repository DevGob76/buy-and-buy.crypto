...
const pageInitializers = {
    'repartition': () => {
        ...
        document.getElementById('archive-date').valueAsDate = new Date();
    ...
    'pret-trading': () => {
        document.getElementById('loan-archive-date').valueAsDate = new Date();
...
function showPage(pageId) {
    ...
    const navButton = document.querySelector(`.nav-btn[onclick*="'${pageId}'"]`);
    if(navButton) {
        navButton.classList.add('active');
    }

    // Run initializer for the page
    if (pageInitializers[pageId]) {
        pageInitializers[pageId]();
        // To prevent re-initialization
        delete pageInitializers[pageId];
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Initial active page is 'accueil', which has no specific JS initialization needed upfront.
    // Event listeners on nav buttons handle other page initializations via showPage.
});